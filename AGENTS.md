...
2. Ingestion flow: Codex calls GraphQL `ingestDocuments`; the service resolves the file under `DOCUMENT_ROOT`; OpenSearch is checked for the file name; existing files are skipped by default, and new files are parsed, chunked, enriched with headings/keywords, and indexed. Do not use `force: true` unless the user explicitly asks to reindex or overwrite existing indexed records.
